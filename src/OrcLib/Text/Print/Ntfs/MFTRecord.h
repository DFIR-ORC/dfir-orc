...
template <typename T>
void PrintFilenameAttribute(Orc::Text::Tree<T>& root, const Orc::MFTRecord& record)
{
...
template <typename T>
void Print(Orc::Text::Tree<T>& root, const std::string& name, const IndexRootAttribute& attributes)
{
...
template <typename T>
void PrintCarvedFileEntries(Orc::Text::Tree<T>& root, std::string_view buffer)
{
    uint64_t carvedEntryCount = 0;

    for (auto offset = buffer.data(); offset + sizeof(FILE_NAME) < buffer.size(); ++offset)
    {
        PFILE_NAME file_name = reinterpret_cast<PFILE_NAME>(offset);
...
template <typename T>
void Print(Orc::Text::Tree<T>& root, const std::string& name, const BitmapAttribute& attributes)
{
    auto node = root.AddNode(name);
    PrintValue(node, "Name", std::wstring_view(attributes.NamePtr(), attributes.NameLength()));
    PrintValue(node, "Bitmap", fmt::format("0x{}", detail::ToHexString(attributes.Bits())));
    root.AddEmptyLine();
}
...
template <typename T>
void PrintReparsePointAttribute(Orc::Text::Tree<T>& root, const Orc::MFTRecord& record)
{
...
template <typename T>
void PrintDataAttribute(Orc::Text::Tree<T>& root, const MFTRecord& record, const std::shared_ptr<VolumeReader>& volume)
{
...
void Print(Orc::Text::Tree<T>& root, const MFTRecord& record, const std::shared_ptr<VolumeReader>& volume)
{
    ...
    detail::PrintStandardInformation(recordNode, record);
    detail::PrintFilenameAttribute(recordNode, record);
    detail::PrintIndexAttributes(recordNode, record, volume);
    detail::PrintExtendedAttributes(recordNode, record, volume);
    detail::PrintReparsePointAttribute(recordNode, record);
    detail::PrintDataAttribute(recordNode, record, volume);
}
