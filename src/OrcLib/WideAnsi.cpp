    ...
    __in DWORD cchDest)
{
        ...
        || pszDest[0] == 0)
    {
        ...
        Log::Error("WideCharToMultiByte failed [{}]", SystemError(hr));
        return hr;
    }
    if (cchSize < cchDest)
        pszDest[cchSize] = L'\0';
...
Orc::AnsiToWide(__in PCSTR pszSrc, __out_ecount(cchDest) PWSTR pwzDest, __in DWORD cchDest)
{
    ...
    {
        hr = HRESULT_FROM_WIN32(GetLastError());
        Log::Error("MultiByteToWideChar failed [{}]", SystemError(hr));
        return hr;
    }
...
HRESULT Orc::AnsiToWide(__in std::string_view src, std::wstring& dest)
{
    ...
    {
        hr = HRESULT_FROM_WIN32(GetLastError());
        Log::Error(L"MultiByteToWideChar failed [{}]", SystemError(hr));
        return hr;
    }
