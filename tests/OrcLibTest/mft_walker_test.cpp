...
public:
    ...
    TEST_METHOD(MFTWalkerBasicTest)
    {
        m_NbFiles = 0;
        m_NbFolders = 0;
        ProcessArchive(_L_, helper.GetDirectoryName(__WFILE__) + L"\\ntfs_images\\ntfs.7z");
...
private:
    ...
    void ProcessArchive(const logger& pLog, const std::wstring& archive)
    {
        ...
        callBacks.FileNameAndDataCallback = [this, &walker, &pLog](
            ...
            Authenticode authenticode(pLog);
